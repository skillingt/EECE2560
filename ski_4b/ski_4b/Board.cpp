...
bool Board::SolveBoard(int i, int j)
// Attempts to solve the board
{
    ...
    if (i >= 1 && i <= boardSize && j >= 1 && j <= boardSize)
    {
        ...
        // Return what square the current index is in
        square = SquareNumber(i,j);
        // If the cell is blank, try and place a digit
        if (index == blank)
        {
            // Future optimization: Check to see if there is only one digit to place
            for (int k = 1; k < 10; k++)
            {
                // Check Conflicts
                if (row_con[i][k] == true || col_con[j][k] == true || square_con[square][k] == true)
                {
                    // Try the next digit
                    continue;
                }
                ...
                {
                    // Valid cell, place the digit
                    SetCell(i,j, k);
                    // Update conflicts
                    row_con[i][k] = true;
                    col_con[j][k] = true;
                    square_con[square][k] = true;
                    // See if we have iterated through all rows and columns
                    if (i == 9 && j == 9) {
                        return 1;
                    }
                    // Try to place a digit in the next cell
                    if (j == 9)
                    {
                        if (SolveBoard(i + 1, 1)) return 1;
                    }
                    else
                    {
                        if (SolveBoard(i, j + 1)) return 1;
                    }
                    // Failed to place a valid number, undo previous actions
                    SetCell(i, j, blank);
                    row_con[i][k] = false;
                    col_con[j][k] = false;
                    square_con[square][k] = false;
                } // end try valid digit
            } // end digit for loop
            // Unable to place a digit, back track
            return 0;
        } // end try to place a digit
        // See if we have iterated through all rows and columns
        if (i == 9 && j == 9) {
            return 1;
        }
        // Digit is already there, try to place a digit in the next cell
        if (j == 9)
        {
            if (SolveBoard(i + 1, 1)) return 1;
        }
