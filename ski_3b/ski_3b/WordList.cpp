...
// Copies the items into a heap, uses Heap Sort, and copies the items back into the WordList object
void WordList::HeapSort(){
    // Declare the heap
    Heap<string> my_heap;
    // Copy the unsorted words into the heap (assumes that ReadWords() has been called)
    my_heap.InitializeMaxHeap(word_list);
    // Sort the heap
    my_heap.Heapsort(my_heap.size());
    // Copy the heap into the word list
    word_list = my_heap.GetHeap();
}
...
bool WordList::Search(string key)
{
    bool result;
