...
//  ski_6b
//
//  Created by Taylor Skilling on 12/13/15.
...
// Sets sf equal to a minimum spanning forest on graph g using Prim’s algorithm. Starts at node 0 and uses a priority queue based on a min-heap to store nodes that have not yet been added to the spanning forest
{
    // Clear visited nodes on both graphs
    clearVisited(g);
    clearVisited(sf);
    ...
    // Set key value of the first vertex to 0 so that it is extracted first
	g[0].weight = 0;
    ...
    while (minHeap.size() != 0)
    {
        ...
        if (g[u].weight != 0)
        {
            ...
            pair<Graph::edge_descriptor, bool> checkEdge = edge(g[u].pred, u, g);
            // Add edge
            if (checkEdge.second == true)
            {
                e.weight = g[checkEdge.first].weight;
                add_edge(g[u].pred, u, e, sf);
            }
        }
        if (debug)
            cout << "Extracted node " << u << endl;
        pair<Graph::adjacency_iterator, Graph::adjacency_iterator> aItrRange = adjacent_vertices(u, g);
        for (Graph::adjacency_iterator aItr = aItrRange.first; aItr != aItrRange.second; ++aItr)
        {
            // Assign the adjecant node to v
            v = *aItr;
            if (debug)
                cout << "Checking if adjacent node " << v << " is in minHeap" << endl;
            // Check if v is in min heap
            try
            {
                index = minHeap.getIndex(v);
                if (debug)
                    cout << "Got min heap index " << index << " for node " << v << endl;
                // Get the edge between u and v
                pair<Graph::edge_descriptor, bool> checkEdge = edge(u, v, g);
                // If the adjacent node v is not yet included in MST (still in min heap) and weight of the edge u-v is less than key value of v, then update key value and parent of v
                if (debug)
                    cout << "Checking if " << g[checkEdge.first].weight << " < " << g[v].weight << endl;
                if (g[checkEdge.first].weight < g[v].weight)
                {
                    if (debug)
                        cout << "Updating key, " << g[checkEdge.first].weight << " was less than " << g[v].weight << endl;
                    // Update the key value
                    g[v].weight = g[checkEdge.first].weight;
                    // Update predecessor
                    g[v].pred = u;
...
void printEdges(const Graph sf)
{
    ...
    cout << "The forest has " << conn_comp << " connected components" << endl;
}
...
int main(int argc, const char * argv[])
{
    try
    {
        ...
        string filePath = "/Users/TaylorSkilling/Documents/Junior NU/Fund. of Algorithims/ski_6b/ski_6b/";
        ...
        // Reinitialize sf as the original graph g
        sf = g;
        
        // Remove all the edges from graph sf
        Graph::vertex_descriptor targetV, sourceU;
        pair<Graph::edge_iterator, Graph::edge_iterator> eItrRangeMSF = edges(sf);
		Graph::edge_iterator eItr = eItrRangeMSF.first;
		while (eItr != eItrRangeMSF.second) {
			targetV = target(*eItr, sf);
			sourceU = source(*eItr, sf);
			eItr++;
			remove_edge(sourceU, targetV, sf);
		}
        ...
        std::vector<int> component(num_vertices(sf));
        int conn_comp = connected_components(sf, &component[0]);
        cout << "The MSF has " << conn_comp << " connected components" << endl;
    }
